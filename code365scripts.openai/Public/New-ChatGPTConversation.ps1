function New-ChatGPTConversation {
    ...
    if ($environment) {
        ...
        $parsed_env_config = ($env_config | ConvertFrom-Json).profiles | Where-Object { $_.name -eq $environment } | Select-Object -First 1
        ...
        if ($parsed_env_config) {
            ...
            if ($parsed_env_config.config) { 
                if ($config) {
                    Merge-Hashtable -table1 $parsed_env_config.config -table2 $config
                }
                else {
            ...
            if ($parsed_env_config.headers) {
                ...
                if ($headers) {
                    Merge-Hashtable -table1 $parsed_env_config.headers -table2 $headers
                }
                else {
            ...
            if ($parsed_env_config.auth -and ($parsed_env_config.auth.type -eq "aad") -and $parsed_env_config.auth.aad) {
                $aad =  $parsed_env_config.auth.aad | ConvertTo-HashTable
